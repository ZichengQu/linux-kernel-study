...
#
# 观测 scx_simple 是否已经启用，以及任务是否正在由 sched_ext 管理。
# 脚本会默认创建在线 CPU 数量的 nice -10 任务和同等数量的 nice 10 任务。
...
scx_show_state.py 和两组 nice 正负 10 测试负载来观测 scx_simple。
...
# 打印 scx_simple 的关键 sched_ext 事件计数，聚焦基础诊断项。
print_events()
{
        local events="$SCHED_EXT/simple/events"
...
# 返回测试负载要绑定的 CPU；默认 CPU1，不存在时退回 CPU0。
select_workload_cpu()
{
        if [[ -d "/sys/devices/system/cpu/cpu$WORKLOAD_CPU" ]]; then
                echo "$WORKLOAD_CPU"
        else
                echo 0
        fi
...
# 获取当前在线逻辑 CPU 数量，用于决定每组测试负载数量。
get_online_cpu_count()
{
        getconf _NPROCESSORS_ONLN
}

# 启动一个绑定到指定 CPU 和 nice 值的 CPU-bound 测试任务。
launch_one_workload()
{
        local cpu=$1
        local nice_value=$2

        taskset -c "$cpu" nice -n "$nice_value" bash -c 'while :; do :; done' &
        work_pids+=("$!")
        work_nices+=("$nice_value")
}

# 创建两组绑定到同一 CPU 的 CPU-bound 任务，用 nice 正负 10 观察权重差异。
launch_workload()
{
        local cpu idx
        ...
        cpu=$(select_workload_cpu)
        selected_workload_cpu=$cpu
        nr_cpus=$(get_online_cpu_count)

        if ((EUID != 0)); then
                echo "提示：当前用户无法设置 nice -10，高权重测试任务将使用 nice 0。" >&2
        fi

        for ((idx = 0; idx < nr_cpus; idx++)); do
                if ((EUID == 0)); then
                        launch_one_workload "$cpu" -10
                else
                        launch_one_workload "$cpu" 0
                fi
        done
        ...
        echo "workload cpu: $cpu"
        echo "online cpus:  $nr_cpus"
        echo "nice -10 tasks: $nr_cpus"
        echo "nice  10 tasks: $nr_cpus"
...
# 启动默认测试负载，并选择第一个高权重任务作为默认观测 PID。
setup_workload()
{
        launch_workload
...
print_header()
{
        ...
        echo "=== scx_simple observation: $now ==="
        echo
}
...
# 打印 scx_simple 对应的 sched_ext 事件计数，用于发现 fallback/refill 等行为。
print_events_section()
{
        echo "[4] sched_ext/simple events"
        print_events
        echo
}

# 提醒用户 scx_simple 的 local/global 计数需要在其运行终端观察。
print_scx_simple_stdout_hint()
{
        echo "[5] scx_simple stdout"
        echo "本脚本不启动 scx_simple；请在运行 scx_simple 的终端观察 local/global 计数。"
        echo
}

# 从 /proc/stat 读取每个 CPU 的 total/idle 计数，用于计算采样窗口。
read_cpu_stats()
{
...
# 打印测试负载的 ext 状态和采样窗口 CPU 使用快照，用于粗略对比 nice 权重影响。
print_workload_section()
{
        ...
        echo "[6] nice +/-10 workload"
        echo "bound_cpu=$selected_workload_cpu"
        echo "online_cpus=$nr_cpus nice_-10_tasks=$nr_cpus nice_10_tasks=$nr_cpus"
        for idx in "${!work_pids[@]}"; do
                echo "task[$idx] nice=${work_nices[$idx]} pid=${work_pids[$idx]}"
